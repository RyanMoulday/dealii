...
/**
 * Like mg_transfer_a_02.cc but for MGTwoLevelTransferCopyToHost.
 */
...
do_test(const FiniteElement<dim> &fe_fine, const FiniteElement<dim> &fe_coarse)
{
  ...
  MGTwoLevelTransferCopyToHost<dim, LinearAlgebra::distributed::Vector<Number, MemorySpace>> transfer;
  transfer.reinit(dof_handler_fine,
                  dof_handler_coarse,
                  constraint_fine,
                  constraint_coarse);

  test_copy_to_host_transfer_operator(transfer, dof_handler_fine, dof_handler_coarse);
}
...
test(int fe_degree)
{
...

  {
    ...
    do_test<dim, Number, MemorySpace>(FE_Q<dim>(fe_degree), FE_Q<dim>(fe_degree));
    deallog.pop();
  }

  {
    deallog.push("DG<2>(" + str_fine + ")<->CG<2>(" + str_coarse + ")");
    do_test<dim, Number, MemorySpace>(FE_DGQ<dim>(fe_degree), FE_Q<dim>(fe_degree));
    deallog.pop();
  }

  {
    deallog.push("DG<2>(" + str_fine + ")<->DG<2>(" + str_coarse + ")");
    do_test<dim, Number, MemorySpace>(FE_DGQ<dim>(fe_degree), FE_DGQ<dim>(fe_degree));
    deallog.pop();
  }
}
...
main(int argc, char **argv)
{
  ...
  test<2, double, MemorySpace::Default>(2 /*=degree*/);
}
