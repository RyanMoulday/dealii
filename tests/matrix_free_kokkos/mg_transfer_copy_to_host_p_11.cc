...
/**
 * Like mg_transfer_p_11.cc but for MGTwoLevelTransferCopyToHost.
 */
        ...
        const Triangulation<dim> &tria)
{
  ...
  mf_coarse.initialize_dof_vector(tmp, 1);
...
test(int fe_degree_fine, int fe_degree_coarse)
{
  ...
  do_test<dim, Number, MemorySpace>(
    FESystem<dim>(FE_Q<dim>(fe_degree_fine), dim),
    FESystem<dim>(FE_Q<dim>(fe_degree_coarse), dim),
    tria);
  do_test<dim, Number, MemorySpace>(
    FESystem<dim>(FE_DGQ<dim>(fe_degree_fine), dim),
    FESystem<dim>(FE_Q<dim>(fe_degree_coarse), dim),
    tria);
  do_test<dim, Number, MemorySpace>(
    FESystem<dim>(FE_DGQ<dim>(fe_degree_fine), dim),
  ...
  do_test<dim, Number, MemorySpace>(
    ...
    tria);
  do_test<dim, Number, MemorySpace>(
    FESystem<dim>(FE_DGQHermite<dim>(fe_degree_fine), dim),
    FESystem<dim>(FE_DGQ<dim>(fe_degree_coarse), dim),
    tria);
}
